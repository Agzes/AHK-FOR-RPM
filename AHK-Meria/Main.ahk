...
licens_advo(GuiObject?, eventInfo?){
    ...
    SendInput("{t}")
    Sleep(300)
    SendInput("/do В закрытом канале реестра появилась лицензия частного адвоката для: " licens_name " на номер паспорта RPM-" licens_pass "{Enter}")
}
...
vlicens_advo(GuiObject?, eventInfo?){
    ...
    SendInput("{t}")
    Sleep(300)
    SendInput("/do В закрытом канале реестра появилась лицензия частного адвоката для: " vlicens_name " на номер паспорта RPM-" vlicens_pass "{Enter}")
}
...
cudia.Add("Button", "w300 x5 y180" , "Alt+P | выдать паспорт").OnEvent("Click", give_pass)
cudia.Add("Button", "w300 x5 y215" , "Alt+C | изменить паспорт").OnEvent("Click", change_pass)
cudia.AddText("cWhite   x5 y250","   AHK | Meria | сделано Agzes[WertyKnack] с ❤")
