...
AHK_version := "30.10.25 • Основан на отыгровках от Mirml"
code_version := 1
Font := "Segoe UI"
FontSize := 11
...
InitGBinds(i) {
    i["AskPassport"]         := ['!2', "Попросить паспорт (ФБК/ДКП)"]
    i["AskToNotDisturb"]     := ['!y', "Попросить не нарушать"]
    i["ChangePassport"]      := ['!c', "Изменить паспорт"]
    i["ForceStop"]           := ['Insert', "[ForceStop] - Остановка отыгровок"]
    i["GivePassport"]        := ['!p', "Выдать паспорт новичку"]
    i["GoToMeeting"]         := ['!5', "Суд: Уйти на совещание"]
    i["Greetings"]           := ['!q', "Приветствие"]
    i["PresentOfficially"]   := ['!1', "Представиться официально"]
    i["Restart"]             := ['!r', "Перезагрузка"]
    i["RulesTrial"]          := ['!4', "Суд: Правила"]
    i["StartTrial"]          := ['!3', "Суд: Начало"]
    i["TakePassportAndStar"] := ['!k', "Взять паспорт и спросить о задержании"]
    i["UDO_Give"]            := ['!d', "Выдать УДО"]
    i["UDO_Price"]           := ['!n', "Цены на УДО"]
    i["UI_Main"]             := ['F4', "[UI] Основное"]
    i["UI_Menu"]             := ['F9', "[UI] Меню"]
    i["UpBillboard"]         := ['!b', "Развесить биллборд"]
    i["UseForce"]            := ['!h', "Применить силу"]
}

InitGBindsCfg(i) {
    i["G_Department"] := "Отсутствует"
    i["G_CertificateDate"] := "30.10.2025"
    i["G_DepartmentRole"] := ""
}

InitGBindsCfgUI() {
    RpSetUIGen(RpSetUI, "Input", "Отдел", "G_Department", "Выберите ваш отдел: ФБК / ДКП / Отсутствует")
    RpSetUIGen(RpSetUI, "Input", "Дата выдачи удостоверения", "G_CertificateDate", "Дата, которая будет отображаться в удостоверении.")
    RpSetUIGen(RpSetUI, "Input", "Роль в отделе", "G_DepartmentRole", "Ваша должность в отделе (если применимо).")
}

InitRPActions(i) {
    i["Greetings"] := ["Greetings", "RPAction", [["Chat","Здравствуйте, меня зовут " Name ". Чем могу помочь?{ENTER}",S300,S300]]]
    i["PresentOfficially"] := ["PresentOfficially", "Func", present_officially]
    i["AskPassport"] := ["AskPassport", "Func", ask_passport]
    i["UseForce"] := ["UseForce", "Func", use_force]
    i["AskToNotDisturb"] := ["AskToNotDisturb", "Func", ask_to_not_disturb]
    i["GivePassport"] := ["GivePassport", "Func", give_passport]
    i["ChangePassport"] := ["ChangePassport", "Func", change_passport]
    i["UpBillboard"] := ["UpBillboard", "Func", up_billboard]
    i["UDO_Price"] := ["UDO_Price", "Func", udo_price]
    i["UDO_Give"] := ["UDO_Give", "Func", udo_give]
    i["TakePassportAndStar"] := ["TakePassportAndStar", "Func", take_passport_and_star]
    i["StartTrial"] := ["StartTrial", "Func", start_trial]
    i["RulesTrial"] := ["RulesTrial", "Func", rules_trial]
    i["GoToMeeting"] := ["GoToMeeting", "Func", go_to_meeting]
    i["HelpFunc_OpenWindow"] := ["UI_Main", "Func", open_role_window]
}

GBindsSortedArrayForSet := ["AskPassport", "AskToNotDisturb", "ChangePassport", "ForceStop", "GivePassport", "GoToMeeting", "Greetings", "PresentOfficially", "Restart", "RulesTrial", "StartTrial", "TakePassportAndStar", "UDO_Give", "UDO_Price", "UI_Main", "UI_Menu", "UpBillboard", "UseForce"]
GBindsSortedArray := ["AskPassport", "AskToNotDisturb", "ChangePassport", "GivePassport", "GoToMeeting", "Greetings", "PresentOfficially", "RulesTrial", "StartTrial", "TakePassportAndStar", "UDO_Give", "UDO_Price", "UpBillboard", "UseForce", "UI_Main"]

GWindows := [
    ["Custom", "Secretary", "Секретарь", "None", ["Greetings", "PresentOfficially", "AskPassport", "UseForce", "AskToNotDisturb", "GivePassport", "ChangePassport"]],
    ["Custom", "Chairman", "Председатель", "None", ["Greetings", "PresentOfficially", "AskPassport", "UseForce", "AskToNotDisturb", "GivePassport", "ChangePassport", "UpBillboard"]],
    ["Custom", "Lawyer", "Адвокат", "None", ["Greetings", "PresentOfficially", "AskPassport", "UseForce", "AskToNotDisturb", "GivePassport", "ChangePassport", "UDO_Price", "UDO_Give", "TakePassportAndStar"]],
    ["Custom", "Judge", "Судья", "None", ["Greetings", "PresentOfficially", "AskPassport", "UseForce", "AskToNotDisturb", "GivePassport", "ChangePassport", "UDO_Price", "TakePassportAndStar", "StartTrial", "RulesTrial", "GoToMeeting"]],
    ["Custom", "ViceGovernor", "Вице-Губернатор", "None", ["Greetings", "PresentOfficially", "AskPassport", "UseForce", "AskToNotDisturb", "GivePassport", "ChangePassport", "UpBillboard", "UDO_Price"]],
    ["Custom", "Governor", "Губернатор", "None", ["Greetings", "PresentOfficially", "AskPassport", "UseForce", "AskToNotDisturb", "GivePassport", "ChangePassport", "UpBillboard", "UDO_Price"]],
    ["Custom", "Default", "Водитель", "None", ["Greetings", "PresentOfficially", "UseForce", "AskToNotDisturb"]]
]
    ...
    if (L_Gender = "Женский") {
        if (Department = "Отсутствует") {
            RPAction([
                ...
                ["Chat", "/do Удостоверение: ИФ: " Name " | Фото 3x4 | Организа ция: Правительство | Должность: " Role " | Дата выдачи: " CertDate " | Отдел: " Department " | Подпись губернатора | Штамп MERIA{ENTER}", S100, S100],
...
ask_passport(Element?, *) {
    Department := G_Binds_cfg["G_Department"]
    
    if (Department = "Отсутствует") {
        MsgBox("Извините, у вас отсутствует отдел проверки паспорта ФБК или ДКП. `n Заполнить данные можно в Параметрах -> Отыгровки.")
    } else if (Department = "ФБК" or Department = "ДКП") {
        RPAction([
            ["Chat", "Здравствуйте, я сотрудник отдела " Department ", " Name ". Покажите мне ваш паспорт на проверку.{ENTER}", S100, S100],
            ["Chat", "/pass ", S100, S100]
        ])
    }
}

ask_to_not_disturb(Element?, *) {
    L_Gender := Gender = 1 ? "Мужской" : "Женский"
    if (L_Gender = "Женский") {
        RPAction([
...
udo_price(Element?, *) {
    ...
    if InStr(allowed_positions, "|" . Role . "|") {
        ...
        MsgBox("Извините, вы не можете называть цены УДО.")
    }
}
...
udo_give(Element?, *) {
    ...
    if (Role = "Адвокат") {
        ...
        MsgBox("Извините, вы не можете выдавать УДО.")
    }
}

take_passport_and_star(Element?, *) {
    ...
    if InStr(allowed_positions, "|" . Role . "|") {
        ...
        MsgBox("Извините, данное действие может выполнять только адвокат или судья.")
    }
}
...
; AFR v.2.2 CODE PART (Не изменяйте для корректной работы конфига)
{
    ...
    UpdateAHK(Element?, *) {
        Run("https://github.com/Agzes/AHK-FOR-RPM/releases/")
        UpdateUI.Hide()
    }
